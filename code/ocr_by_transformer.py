...
class OCR_EncoderDecoder(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        ...
        memory = self.encode(src, src_mask)  # [64,24,512]
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask):
        # memory.shape=[64,24,512]
        target_embedds = self.tgt_embed(tgt)  # [64,20,512]
...
def run_epoch(data_loader, model, loss_compute, device=None):
    ...
    for i, batch in enumerate(data_loader):
        ...
        encode_mask = encode_mask.to(device)  # [64,1,24]
        decode_in = decode_in.to(device)  # [64,20]
...
if __name__ == "__main__":
    ...
    train_max_label_len = statistics_max_len_label(
        train_lbl_path)  # train_max_label_len = 19
    valid_max_label_len = statistics_max_len_label(
        valid_lbl_path)  # valid_max_label_len = 21
